  ...
  def prepare(sampler: Sampler, warmupIterations: Int, syntheticSamples: Int)(
    ...
    val thin =
      if (minEffectiveSampleSize < Samples)
        ...
        initialThin
    SBCRun(this, sampler, warmupIterations, syntheticSamples, thin, maxRHat)
  }
...
case class SBCRun(sbc: SBC[_],
  ...
  private def rankStream(bins: Int, remaining: Int): Stream[Int] =
    if (remaining == 0)
      ...
      val (rawRank, _, _) =
        sbc.sample(sampler, warmupIterations, syntheticSamples, thin)
  ...
  private def binomialQuantile(q: Double, n: Int, p: Double): Int = {
    ...
    while (cmf < q) {
      ...
      cmf += sbc.emptyEvaluator.toDouble(logPmf.exp)
    }
    k
  }
}
