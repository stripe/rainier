...
case class Prediction[T](model: Model, generator: Generator[T]) {
    ...
    def toSample(sampler: Sampler, warmupIterations: Int, iterations: Int, keepEvery: Int = 1, nChains: Int = 1)(implicit rng: RNG): Sample[T] = {
        val density = model.density
        val chains = 1.to(nChains).toList.map{_ => sampler.sample(density, warmupIterations, iterations, keepEvery)}
