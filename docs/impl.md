...
In order to support gradient-based sampling and optimization methods, `Real` provides `gradient`, which uses reverse-mode autodifferentiation to construct the gradient for each `Variable` leaf in the DAG with respect to the output node (in the form of a new, transformed `Real` for each variable).
...
`RandomVariable` also provides monadic `map` and `flatMap` methods, but with a necessarily quite different implementations from `Generator`.  `map(f: T => U)` returns a new `RandomVariable[U]` whose `density` is unchanged but whose `value` function is replaced with some new object which is a transformation via `f` of its old value function. `flatMap(f: T => RandomVariable[U])` 

 both of which return new `RandomVariable` objects. is the composition of the old `value` function with `f`; it's a new output transformation without affecting the underlying parameter distribution.

[value and density representations]
